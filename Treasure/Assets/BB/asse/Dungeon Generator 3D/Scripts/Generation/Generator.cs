...
public class Generator : MonoBehaviour
{
    ...
    public Transform enemyParent; // Enemies オブジェクト
    ...
    private void SpawnEnemiesRandom()
    {
        ...
        {
            // ランダムな Room を選ぶ
            Room randRoom = parentRooms[Random.Range(0, parentRooms.Length)];

            // Room のランダム位置
            Vector3 spawnPos = randRoom.transform.position;
            ...
            {
                Instantiate(enemyPrefab, hit.position, Quaternion.identity, enemyParent);
            }
        }
    }
    ...
    private void Generate()
    {
        ...
        SpawnEntities();
        navMeshSurface.BuildNavMesh(); // ←NavMeshBake
    ...
    private void CreateCorridor(int corridor, bool calculateNext, bool isFirstTime, Transform target, Transform startPoint)
    {
        ...
        if (corridor == 0)
        {
            ...
            do
            {
                ...
                // ===== 範囲チェック（最重要）=====
                if (tempIntX < 0 || tempIntY < 0 ||
                    tempIntX >= area.GetLength(0) ||
                    tempIntY >= area.GetLength(1))
                {
                    // マップ外に出たら廊下生成を中断
                    return; // または break;
                }
                ...
                corridorFloor = Instantiate(
                    flooring[randomFloor].gameObject,
                    new Vector3(
                        startPoint.position.x,
                        0,
                        startPoint.position.z + additive
                    ),
                    Quaternion.identity
                );
        ...
        else if(corridor == 1) // Do Y corridor
        {
            ...
            do
            {
                ...
                if (IsInside(tempIntX, tempIntY) && area[tempIntX, tempIntY] != null)
                {
                    DestroyImmediate(corridorFloor);
                    corridorFloor = area[tempIntX, tempIntY];
                }
            ...
            if (obj != null)
            {
                ...
                else if (obj.tag == "Wall")
                {
                    ...
                    if (CheckNullAndTag("Floor", true, area[a - 1, b], area[a - 1, b - 1], area[a, b - 1]))
                    {
                        ...
                        if(a <= 0 || b <= 0 || a >= area.GetLength(0) - 1 || b >= area.GetLength(1) -1)
                        {
                            continue;
                        }
    ...
    {
        // ===== 完全ガード（最重要）=====
        if (!IsInside(a, b)) return;
        if (area[a, b] == null) return;

        bool isXBigger = false;

        // ================= LEFT =================
        if (IsInside(a - 1, b) && CheckIfNull(area[a - 1, b])) // ★安全
        {
            int angle = 0;
            int randomWall = Random.Range(0, walls.Count);

            float displacement = CheckWallSize(walls[randomWall].gameObject, ref isXBigger) / 2f;
            if (isXBigger) angle = 90;

            area[a - 1, b] = Instantiate(
                walls[randomWall].gameObject,
                new Vector3(a - 0.5f - displacement, walls[randomWall].gameObject.transform.position.y, b),
                Quaternion.Euler(0, angle, 0)
            );

            area[a - 1, b].name = walls[randomWall].name + " 1 A";

            if (IsInside(a - 2, b) && CheckNullAndTag("Floor", true, area[a - 2, b])) // ★安全
            {
                GameObject localObject = Instantiate(
                    walls[randomWall].gameObject,
                    new Vector3(a - 1.5f + displacement, walls[randomWall].gameObject.transform.position.y, b),
                    Quaternion.Euler(0, angle, 0)
                );
        ...
        // ================= DOWN =================
        if (IsInside(a, b - 1) && CheckIfNull(area[a, b - 1])) // ★安全
        {
            int angle = -90;
            int randomWall = Random.Range(0, walls.Count);
            ...
            area[a, b - 1] = Instantiate(
                walls[randomWall].gameObject,
                new Vector3(a, walls[randomWall].gameObject.transform.position.y, b - 0.5f - displacement),
                Quaternion.Euler(0, angle, 0)
            );

            area[a, b - 1].name = walls[randomWall].name + " 2 A";

            if (IsInside(a, b - 2) && CheckNullAndTag("Floor", true, area[a, b - 2])) // ★安全
            {
                GameObject localObject = Instantiate(
                    walls[randomWall].gameObject,
                    new Vector3(a, walls[randomWall].gameObject.transform.position.y, b - 1.5f + displacement),
                    Quaternion.Euler(0, angle, 0)
                );
